...
extern void drv_bootusb_init(void); /* boot usb implementation */
extern void bootusb_loop(void);

#ifdef CONFIG_USB_STORAGE
static int usb_stor_curr_dev=-1; /* current device */
#endif

#if !defined(CONFIG_USB_BOOTUSB)
...
#else 	/* CONFIG_USB_BOOTUSB */
	if (strncmp(argv[1], "boot", 4) == 0) {
		drv_bootusb_init();
		bootusb_loop();
		return 0;
	}
...
#elif !defined(CONFIG_USB_BOOTUSB)
U_BOOT_CMD(
	usb,	5,	1,	do_usb,
	"usb     - USB sub-system\n",
	"reset - reset (rescan) USB controller\n"
...
U_BOOT_CMD(
	usb,	5, 1, do_usb,
	"usb	- USB sub-system\n",
	"boot   - boot using boot_usb\n"
	"info	- show information about client\n");
#endif
#endif
