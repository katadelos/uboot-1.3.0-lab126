...
/*
 * Workaround to update self : Fixed the structure during initialization
 *  The memory management structure should be changed to create in fly
 */
int malloc_bin_init(void)
{
	int i;
	if (av_[0] || av_[1]) {
		av_[0] = av_[1] = 0;
		av_[2] = av_[3] = (mbinptr)av_;
		for (i = 4; i < ((sizeof(av_) / sizeof(mbinptr))); i++)
			av_[i] = bin_at((i-2) / 2);
	}
	return 0;
}
